...
import {createOaWop} from "@/api/bpm/oaWop";
...
export default {
  ...
  data() {
    return {
      ...
      typeDictData: getDictDatas(DICT_TYPE.BPM_OA_OWP_TYPE),
    };
  },
  created() {
  },
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createOaWop(this.form).then(response => {
          this.$modal.msgSuccess("发起成功");
          this.$tab.closeOpenPage({ path: "/bpm/oa/oa-wop" });
        });
      });
    }
  }
